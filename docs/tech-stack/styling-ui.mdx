...
Installed primitives (`src/components/ui/`): `alert-dialog`, `badge`, `button`, `card`, `collapsible`, `dialog`, `dropdown-menu`, `form`, `input`, `label`, `select`, `sheet`, `skeleton`, `sonner`, `table`, `tabs`, `textarea`. Run `pnpm dlx shadcn@latest add <component>` to pull in more.
